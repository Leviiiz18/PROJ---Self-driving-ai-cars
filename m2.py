...
class FixedStagnation(DefaultStagnation):
    def remove_stagnant(self, generation):
        ...
        # Then perform normal stagnation checks
        return super().remove_stagnant(generation)

    def update(self, species_set, generation):
        """Can add custom update logic if needed"""
...
class Car:
    ...
    def get_corners(self):
        ...
        if not all(isinstance(val, (int, float)) and not math.isnan(val) and math.isfinite(val) 
            ...
            return []  # Return empty list if any value is invalid
    ...
    def check_checkpoints(self):
        ...
        if self.passed_checkpoint(checkpoint):
            ...
            # Time-based bonus for faster segments
            if len(self.checkpoint_times) > 1:
                time_diff = (self.checkpoint_times[-1] - self.checkpoint_times[-2]) / 1000
                time_bonus = max(0, 20 - time_diff)
    ...
    def calculate_fitness(self):
        ...
        # Exponential checkpoint rewards
    ...
    def update(self, steering, throttle):
        ...
        # Collision detection and boundary tracking
        center_on_track = self.is_on_track((new_x, new_y))
        
        # Update boundary status before position update
        self.update_boundary_status(center_on_track)
        
        # TODO: Implement distance from center calculation
        # distance_from_center = calculate_distance_from_center(new_x, new_y)
        # self.update_distance_from_center(distance_from_center)
    ...
    def get_sensor_data(self):
        sensor_data = []
        for sensor in self.sensors:
            normalized_distance = sensor["distance"] / SENSOR_LENGTH
            sensor_data.append(normalized_distance)
        return sensor_data


def draw_track(screen):
        pygame.draw.polygon(screen, (100, 100, 100), TRACK_OUTER)  # Changed from TRACK_COLOR (white)
        pygame.draw.polygon(screen, (50, 50, 50), TRACK_INNER)     # Changed from light gray
...
def draw_timer(screen, time_left, font):
...
def draw_fitness_graph(screen, x, y, width, height, stats):
    ...
    pygame.draw.line(graph_surface, (0, 0, 0), (0, height-20), (width, height-20), 2)
    pygame.draw.line(graph_surface, (0, 0, 0), (30, 0), (30, height-20), 2)
    ...
    for f in np.linspace(min_fitness, max_fitness, 5):
        ...
        text = font.render(f"{f:.0f}", True, (0, 0, 0))
        graph_surface.blit(text, (5, y_pos - 8))
    
    prev_x, prev_y = None, None
    for i, stat in enumerate(stats):
        x = 30 + i * x_scale
        y = height - 20 - (stat['best_fitness'] - min_fitness) * y_scale
        
        pygame.draw.circle(graph_surface, (0, 0, 255), (int(x), int(y)), 3)
        
        avg_y = height - 20 - (stat['avg_fitness'] - min_fitness) * y_scale
        pygame.draw.circle(graph_surface, (0, 255, 0), (int(x), int(avg_y)), 3)
        
        if prev_x is not None:
            pygame.draw.line(graph_surface, (0, 0, 255), (prev_x, prev_y), (x, y), 2)
...
def draw_generation_panel(screen, x, y, width, height, current_gen, stats):
    ...
    pygame.draw.rect(panel, (200, 200, 200), (0, 0, width, height), 2)
    
    font = pygame.font.SysFont("Arial", 18)
    small_font = pygame.font.SysFont("Arial", 14)
    
    title = font.render(f"Generation {current_gen + 1} Summary", True, (0, 0, 0))
    ...
    if stats:
        ...
        best_text = font.render("All-Time Best:", True, (0, 0, 0))
...
def draw_realtime_network(screen, car, genome, config):
    ...
    for i, key in enumerate(input_keys):
        x = 40
        y = 40 + i * (net_height - 80) / len(input_keys)
    ...
    for conn_key, conn in genome.connections.items():
        ...
        if in_node in node_positions and out_node in node_positions:
            ...
            if signal_strength > 0.1:
                # Create moving dot along the connection
    ...
    value_font = pygame.font.SysFont("Arial", 8)
    ...
    for key, pos in node_positions.items():
        ...
        text_rect = text.get_rect(center=(int(pos[0]), int(pos[1]) + node_radius + 12))
    ...
    fitness_text = info_font.render(f"Best Car Fitness: {car.fitness:.2f}", True, (255, 255, 255))
    network_surface.blit(fitness_text, (10, 10))
    
    speed_text = info_font.render(f"Speed: {car.speed:.2f}", True, (255, 255, 255))
    network_surface.blit(speed_text, (10, 25))
    ...
    # Input bars
    if hasattr(car.network_state, 'inputs'):
        for i, value in enumerate(car.network_state.inputs):
            ...
            # Background bar
            pygame.draw.rect(network_surface, (50, 50, 50), (bar_x, bar_y, bar_width, bar_height))
            
            # Value bar
            fill_width = int(abs(value) * bar_width)
            bar_color = (0, 255, 0) if value > 0 else (255, 0, 0)
            pygame.draw.rect(network_surface, bar_color, (bar_x, bar_y, fill_width, bar_height))
            
            # Label
            label_text = value_font.render(f"{input_labels[i]}: {value:.2f}", True, (255, 255, 255))
            network_surface.blit(label_text, (bar_x + bar_width + 5, bar_y))
    
    # Output bars
    if hasattr(car.network_state, 'outputs'):
        for i, value in enumerate(car.network_state.outputs):
            if i >= len(output_labels):
                continue
                
            bar_x = net_width - bar_width - 10
            bar_y = 100 + i * 40
            ...
            # Value bar
            fill_width = int(abs(value) * bar_width)
            bar_color = (255, 100, 100) if value > 0 else (100, 100, 255)
            pygame.draw.rect(network_surface, bar_color, (bar_x, bar_y, fill_width, bar_height))
            
            # Label
            label_text = value_font.render(f"{output_labels[i]}: {value:.2f}", True, (255, 255, 255))
            network_surface.blit(label_text, (bar_x - 80, bar_y))
    
    # Blit to main screen
    screen.blit(network_surface, (net_x, net_y))
def eval_genomes(genomes, config):
    for genome_id, genome in list(genomes):  # Use list() to avoid mutation during iteration
        if genome_id not in pop.population:  # Check if genome exists in the population
            print(f"Genome {genome_id} not found in population - skipping")
            continue
    global show_sensors, show_network, manual_generation_advance, generation_stats, best_genome_history, global_best_genome, global_best_fitness, screen
    
    # Get the global screen surface
    ...
    while running and not generation_finished:
        ...
        # Update cars with neural network outputs
        i = 0
        best_car = None
        best_fitness = -1
        ...
        generation_text = font.render(f"Generation: {pop.generation}", True, (255, 255, 255))
        cars_text = font.render(f"Cars Alive: {active_cars_count}", True, (255, 255, 255))
        time_text = font.render(f"Time Left: {time_left:.1f}s", True, (255, 255, 255))
        
        screen.blit(generation_text, (10, 10))
        screen.blit(cars_text, (10, 35))
        screen.blit(time_text, (10, 60))
        
        # Show current best fitness
        if ge:
            current_best_fitness = max(genome.fitness for genome in ge)
            best_fitness_text = font.render(f"Best Fitness: {current_best_fitness:.2f}", True, (255, 255, 255))
        ...
        # Draw real-time network visualization if enabled
        if show_network and best_car:
            draw_realtime_network(screen, best_car, ge[cars.index(best_car) if best_car in cars else 0], config)
...
# Custom reporter to fix the fitness threshold bug
class CustomReporter(neat.reporting.BaseReporter):
    ...
    def end_generation(self, config, population, species_set):
        ...
        if self.generation_start_time is not None:
            ...
            return False  # Never stop due to fitness threshold



def post_evaluate(self, config, population, species, best_genome):
    """Modified to safely handle cases where best genome isn't in species"""
    if best_genome is None:
        print("Warning: No best genome to report")
        return
                 
    try:
        # Only try to get species ID if genome exists in current population
        if best_genome.key in population:
            # Additional check: make sure the species mapping actually contains this genome
            try:
        ...
        # Still report the fitness even if we can't determine species
...
def found_solution(self, config, generation, best):
    ...
    return False  # Don't stop automatically

# Fixed NEAT configuration
...
"""
    ...
    # Initialize config with DefaultStagnation
    config = neat.Config(
        neat.DefaultGenome,
        neat.DefaultReproduction,
        neat.DefaultSpeciesSet,
        neat.DefaultStagnation,  # Using default stagnation
        'neat_config.txt'
    )
    
    # Adjust stagnation parameters programmatically
    config.stagnation_config.max_stagnation = 30  # More generous threshold
    config.stagnation_config.species_elitism = 2  # Keep more species
    
    return config

# Custom Population class to fix fitness threshold bug
...
def run(self, fitness_function, n=None):
    """Modified to ensure we only report on genomes in current population"""
    ...
    while n is None or k < n:
        ...
        # Find best genome in CURRENT population only
        current_best = None
        ...
        # Check for complete extinction again
        if not self.species.species:
            self.reporters.complete_extinction()
            if self.config.reset_on_extinction:
                self.population = self.reproduction.create_new(self.config.genome_type,
...
def run_neat_evolution():
    ...
    try:
        ...
        # Add reporters with progress tracking
...
# Track best genome across generations
        def post_evaluate(config, pop, species, best_genome):
            """Track best genome across generations"""
            global global_best_genome, global_best_fitness  # Changed from nonlocal to global
        ...
        # Run evolution with safety checks
        generation = 0
        while generation < 1000:  # Max generations
            try:
                pop.run(eval_genomes, 1)  # Run one generation at a time
                generation += 1
        ...
        return global_best_genome# Helper function to load and test a saved genome
def test_saved_genome(genome_file='best_genome.pkl'):
    """
    Load and test a previously saved genome
    """
    try:
        ...
        # Test the genome (you can modify this to run a specific test)
        print(f"Testing genome with fitness: {genome.fitness}")
        
        # Run a single test generation with just this genome
        ...
        return None, None
def visualize_best_genome(genome_file='best_genome.pkl'):
    """Visualize the best saved genome driving the track"""
    try:
        with open(genome_file, 'rb') as f:
            genome = pickle.load(f)
        ...
        # Initialize pygame if not already running
        if not pygame.display.get_init():
            pygame.init()
            screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
            pygame.display.set_caption("Best Genome Visualization")
        
        # Create a single car
        car = Car(400, 495, 0, (255, 0, 0))
        clock = pygame.time.Clock()
        running = True
        font = pygame.font.SysFont("Arial", 20)
        
        # Enable visualizations
        global show_sensors, show_network
        show_sensors = True
        show_network = True
        ...
        while running:
            ...
            # Update car
            car.update(output[1], output[0])  # output[1] = steering, output[0] = throttle
            ...
            # Display info
            info_text = [
                f"Fitness: {car.fitness:.2f}",
                f"Speed: {car.speed:.2f}/{car.max_speed}",
            ...
            if show_network:
                car.network_state = NetworkState()
                car.network_state.inputs = inputs
                car.network_state.outputs = output
                draw_realtime_network(screen, car, genome, config)
            ...
            # Stop if car is dead
            if not car.alive:
                print("Car died - ending visualization")
                running = False
